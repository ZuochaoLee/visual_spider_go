...
func Run(id string) {
	...
	//配置数据装载
	base := map[string]string{"taskname": confs.TaskName, "threadnum": strconv.Itoa(confs.Theardnum), "dbtype": confs.Dbtype, "dbhost": confs.Dbhost, "dbport": confs.Dbport, "dbdb": confs.Dbname, "dbuser": confs.Dbuser, "dbpasswd": confs.Dbpasswd}
	conf := map[string]string{"rooturl": confs.RootUrl, "texttype": confs.TextType, "resqType": confs.ReqType, "cookie": confs.Cookie, "headerfile": confs.HeaderFile, "postdata": confs.PostData, "proxy": confs.UseProxy}
